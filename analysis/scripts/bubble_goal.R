...
Var2ColumnName <- "Macro.analysis.goal"
fileName <- "../../paper/figures/goal_year.pdf"
Var1Label <- ""
Var2Label <- ""
bubbleScaler <- 1.1

Var2ColumnOldValues <- c("M", "E", "I")
Var2ColumnNewValues <- c("M", "E", "I")
...
countOccurrences <- function(el1, el2) {
  ...
  for(i in 1:length(Elements1)) {
      if((Elements1[i] == el1) && grepl(Elements2[i], el2)) {
        sum <- sum + 1
      }
  }
  return(sum)
...
Var2 <- unique(Var2ColumnNewValues)
